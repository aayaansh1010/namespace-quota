...
# Function to check ETCD and Velero backup status
check_backup_status() {
  echo "Checking ETCD and Velero backup status..." >> "$output_file"

  # Navigate to /backup directory
  if cd /backup 2>/dev/null; then
    # List ETCD backup status
    echo "1. ETCD Backup Status:" >> "$output_file"
    ls -l >> "$output_file"
    echo >> "$output_file"

    # Check Velero backup status
    echo "2. Velero Backup Status:" >> "$output_file"
    velero get backup >> "$output_file"
    echo >> "$output_file"
  else
    echo "Error: Could not navigate to /backup directory." >> "$output_file"
    echo >> "$output_file"
  fi
}
